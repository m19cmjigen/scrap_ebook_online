...
async function main() {
  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < appConfig.scraper.bookUrls.length; i++) {
        const bookUrl = appConfig.scraper.bookUrls[i];
        const result = await scrapeBook(
          bookUrl,
          i,
          appConfig.scraper.bookUrls.length,
          appConfig,
          storageCoordinator,
          checkpointManager,
          auth
      ...
      if (results.some(r => r.success)) {
        Logger.info('\n✅ Successfully scraped books:');
        results
          .filter(r => r.success)
          .forEach((r, idx) => {
            Logger.info(`  ${idx + 1}. ${r.title || r.bookId}`);
            Logger.info(`     Chapters: ${r.scrapedChapters}/${r.chapterCount}`);
            Logger.info(`     PDF: ${r.outputPath}`);
          });
      }
      ...
      if (results.some(r => !r.success)) {
        ...
        results
          .filter(r => !r.success)
          .forEach((r, idx) => {
            Logger.info(`  ${idx + 1}. ${r.bookUrl}`);
            Logger.info(`     Error: ${r.error?.message || 'Unknown error'}`);
          });
      }
